  ...
  const clearAll = async ()=> {
    if(authenticated) {
      ...
      if(topicName === "ARRAYS 1") {
        ...
        for(let i = 169; i < 176; i++) {
          let reposen = await fetch(`${apiBaseURL}/api/problem-status/${i}/0/`, {
            method: 'GET',
            headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + String(accessToken),
      ...
      >
        <Font family="Secular One">
          ...
          <Box textAlign='center'>
              <Button size="large" sx= {{ textAlign: 'center', border: 1, borderColor: 'text.light' }}
              style={{'marginLeft': '2vh', 'color': 'white' ,'background': 'transparent', 'boxShadow': 'none','borderRadius': '5px'}}
              onClick={() => {
                clearAll();
              }}>
                <b>Clear All<DeleteIcon/></b>
              </Button>
              </Box>
